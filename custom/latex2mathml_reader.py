...
def write_to_file(latex, filename):
    preview('$$%s$$' % (latex,), output='png', viewer='file',
            filename=filename, preamble=PREAMBLE)
...
class LatexToMathMLReader(MarkdownReader):
    ...
    def render_math_img(self, tag):
        ...
        tag.append(
                    E.math(
                        mathml_block,
                        display='block', scriptminsize='21pt', alt=source + ' (click to show image)',
                        xmlns="http://www.w3.org/1998/Math/MathML", 
                        onclick='document.getElementById("%s").classList.remove("latexhidden");' % (name,)),
                    )

        sub_img = E.img(src='/blog/img/%s.png' % (name,), alt=source,
            display='block')
